...
@section('headScripts')
  <link rel="stylesheet" href="{{ secure_asset('css/streams.css').'?'.time() }}">
  <!-- <link rel="stylesheet" href="{{ cdn('css/streams.css').'?'.time() }}"> -->
  <link rel="stylesheet" href="{{ secure_asset('css/comment.css').'?'.time() }}">
  <!-- <link rel="stylesheet" href="{{ cdn('css/comment.css').'?'.time() }}"> -->
  ...
  <!-- <script src="{{ cdn('js/comment.js'.'?'.time()) }}"></script> -->
...
@section('content')
  ...
  <div id="http_events" class="panel panel-default panel-collapse">
    <div class="panel-heading" data-toggle="collapse" data-parent="#http_events" href="#streams" aria-expanded="true" aria-controls="collapseOne" role="tab" id="panel-comment" style="cursor:pointer;">
    ...
    <div class="panel-body collapse in" id="streams">
      <div class="form-inline array-center-livestream-button">
        <div class="filter">
        ...
        <div class="filter pull-right">
          ...
          <input type="checkbox" checked data-toggle="toggle" id="mobile-filter"
                 data-on="All"
                 data-off="<img class='small_icon' src='{{ cdn('icons/streaminfo/mobilecompat.png') }}' >">
        </div>
      </div>
      ...
      <div class="table-responsive">
        <table class="table table-striped table-hover" id="streams-table" width="100%">
          <tbody>
          @if($allStreams->count())
            @if( \Carbon\Carbon::parse($event->start_date)->addHours(Session::get('visitorTZ'))->timestamp - \Carbon\Carbon::now()->addHours(Session::get('visitorTZ'))->timestamp > 3600 && ( Auth::guest() || !Auth::user()->role) )
              <tr>
          ...
          @if($allStreams->count()<1)
            <tr>
              <td colspan="11">There are no streams for this match right now, check again 1 hour before kickoff!</td>
            </tr>
          @endif
          </tbody>
        </table>

				...
				<div class="row clearfix" style="margin-top: 20px;">
	            	<table class="table table-hover table-striped">
	            		<tr>
	            			<th widht=5%>No</th>
	            			<th widht=10%>From</th>
	            			<th class="mobile-view-message-hide" width=20%>Subject</th>
	            			<th class="mobile-view-message-hide" width=40%>Body</th>
	            			<th class="mobile-view-message-date" width=20%>Date</th>
	            			<th width=5%></th>
	            		</tr>
	            		...
	            		@foreach($messages as $message)
	            			@if( $message->type==1 || $message->type==2)
		            			...
		            			<tr class="{{ $unread }}">
                        <td>
                            <a href="<?php echo $message->link . '#' . $message->id ?>" onclick="markAsRead(this, {{ $message->id }})">{{ $i++ }}</a>
                        </td>
	            				  <td>
                            <a href="<?php echo $message->link . '#' . $message->id ?>" onclick="markAsRead(this, {{ $message->id }})">{{ $message->username }}</a>
                        </td>
		            				<td class="mobile-view-message-hide">
                            <a href="<?php echo $message->link . '#' . $message->id ?>" onclick="markAsRead(this, {{ $message->id }})">{{ $message->title }}</a>
                        </td>
		            				<td class="mobile-view-message-hide" style="cursor:pointer;">
                            <a href="<?php echo $message->link . '#' . $message->id ?>" onclick="markAsRead(this, {{ $message->id }})">
		            				...
		            				<td>
                            <a href="<?php echo $message->link . '#' . $message->id ?>" onclick="markAsRead(this, {{ $message->id }})">{{ $message->created_at }}</a>
                        </td>
		            				<td><a href="<?php echo $message->link . '#' . $message->id ?>">
		            					@if( $unread )

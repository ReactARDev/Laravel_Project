...
function check_options() {
	...
	global $icdb, $options;
	$errors = array();
	if ($options['enable_payza'] != "on" && $options['enable_paypal'] != "on" && $options['enable_interkassa'] != "on" && $options['enable_authnet'] != "on" && $options['enable_egopay'] != "on" && $options['enable_perfect'] != "on" && $options['enable_skrill'] != "on" && $options['enable_bitpay'] != "on" && $options['enable_bitcoin'] != "on" && $options['enable_stripe'] != "on") $errors[] = 'Select at least one payment method';
	...
	if ($options['enable_interkassa'] == "on") {
		if (strlen($options['interkassa_checkout_id']) < 3) $errors[] = 'InterKassa Checkout ID is required';
	...
	if ($options['enable_bitcoin'] == "on") {
		if (strlen($options['bitcoin_public']) < 1) $errors[] = 'Bitcoin Public Key is required';
		if (strlen($options['bitcoin_private']) < 1) $errors[] = 'Bitcoin Private Key is required';
	}

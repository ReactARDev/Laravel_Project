		...
		switch ($_GET['action']) {
			...
			case 'update-options':
				...
				if (isset($_POST['action']) && $_POST['action'] == 'update-options') {
					...
					if (isset($_POST["enable_bitcoin"])) $options['enable_bitcoin'] = "on";
					else $options['enable_bitcoin'] = "off";
			...
			case 'update-campaign':
				...
				if (isset($_POST['action']) && $_POST['action'] == 'update-campaign') {
					...
					if (!empty($errors)) {
						...
						if (!empty($id)) {
							$icdb->query("UPDATE ".$icdb->prefix."campaigns SET title = '".$icdb->escape_string($title)."', min_amount = '".number_format(floatval($min_amount), 2, ".", "")."', currency = '".$icdb->escape_string($currency)."', form_intro = '".$icdb->escape_string($form_intro)."', form_terms = '".$icdb->escape_string($form_terms)."', top_intro = '".$icdb->escape_string($top_intro)."', recent_intro = '".$icdb->escape_string($recent_intro)."' WHERE id = '".$id."'");
							$_SESSION['ok'] = 'Campaign details successfully updated!';
						} else {
							$icdb->query("INSERT INTO ".$icdb->prefix."campaigns (title, min_amount, currency, form_intro, form_terms, top_intro, recent_intro, status, details, registered, deleted) VALUES ( '".$icdb->escape_string($title)."', '".number_format(floatval($min_amount), 2, ".", "")."', '".$icdb->escape_string($currency)."', '".$icdb->escape_string($form_intro)."', '".$icdb->escape_string($form_terms)."', '".$icdb->escape_string($top_intro)."', '".$icdb->escape_string($recent_intro)."', '".STATUS_ACTIVE."', '', '".time()."', '0')");
			...
			case 'update-donor':
				...
				if (isset($_POST['action']) && $_POST['action'] == 'update-donor') {
					...
					if (!empty($errors)) {
						...
						if (!empty($id)) {
							$icdb->query("UPDATE ".$icdb->prefix."donors SET campaign_id = '".$campaign_id."', name = '".$icdb->escape_string($name)."', email = '".$icdb->escape_string($email)."', url = '".$icdb->escape_string($url)."', amount = '".number_format(floatval($amount), 2, ".", "")."', currency = '".$icdb->escape_string($currency)."' WHERE id = '".$id."'");
							$_SESSION['ok'] = 'Donor details successfully updated!';
						} else {
							$icdb->query("INSERT INTO ".$icdb->prefix."donors (campaign_id, name, email, url, amount, currency, status, details, registered, deleted) VALUES ('".$campaign_id."', '".$icdb->escape_string($name)."', '".$icdb->escape_string($email)."', '".$icdb->escape_string($url)."', '".number_format(floatval($amount), 2, ".", "")."', '".$icdb->escape_string($currency)."', '".STATUS_ACTIVE."', '', '".time()."', '0')");
	...
	if ($page == 'settings') {
		...
		if (DEMO_MODE) {
			$hidden_options = array (
				...
				"bitcoin_private" => "<hidden>",
				"bitcoin_public" => "<hidden>"
			);
...
?>
		...
		<div class="row">
			...
			<div class="span9">
				...
				<small>Please tick checkbox if you would like to accept payments via Payza/AlertPay.</small>
			</div>
		</div>
		<div class="row">
		...
		<div class="row">
			...
			<div class="span9">
				...
				<small>Please tick checkbox if you would like to accept payments via InterKassa.</small>
			</div>
		</div>
		<div class="row">
			<div class="span3"><strong>Checkout ID:</strong></div>
			<div class="span9">
				<input type="text" class="span9" id="interkassa_checkout_id" name="interkassa_checkout_id" value="<?php echo htmlspecialchars($options['interkassa_checkout_id'], ENT_QUOTES); ?>">
				<br /><small>Please enter valid InterKassa Checkout ID. Ex.: 5294dadebf4efc4f543309eb.</small>
			</div>
		</div>
		<div class="row">
			...
			<div class="span9">
				...
				<br /><small>Please enter Secret/Sign Key. You can get it on <a target="_blank" href="http://new.interkassa.com/account/checkout">checkout settings</a> page.</small>
			</div>
		</div>
		<div class="row">
			<div class="span12">
		...
		<div class="row">
			...
			<div class="span9">
				...
				<small>Please tick checkbox if you would like to accept payments via EgoPay.</small>
			</div>
		</div>
		<div class="row">
		...
		<div class="row">
			...
			<div class="span9">
				<label class="checkbox">
					<input type="checkbox" id="enable_bitcoin" name="enable_bitcoin"<?php echo ($options['enable_bitcoin'] == 'on' ? ' checked="checked"' : ''); ?>"> Accept payments via Bitcoin
				</label>
				<small>Please tick checkbox if you would like to accept payments via Bitcoin.</small>
			</div>
		</div>
		<div class="row">
			<div class="span3"><strong>Public Key:</strong></div>
			<div class="span9">
				<input type="text" class="span9" id="bitcoin_public" name="bitcoin_public" value="<?php echo htmlspecialchars($options['bitcoin_public'], ENT_QUOTES); ?>">
				<br /><small>Please enter valid Public Key. You can get this parameter <a href="https://gourl.io/editrecord/coin_boxes/0" target="_blank">here</a>.</small>
			</div>
		</div>
		<div class="row">
			<div class="span3"><strong>Private Key:</strong></div>
			<div class="span9">
				<input type="text" class="span9" id="bitcoin_private" name="bitcoin_private" value="<?php echo htmlspecialchars($options['bitcoin_private'], ENT_QUOTES); ?>">
				<br /><small>Please enter valid Private Key. You can get this parameter <a href="https://gourl.io/editrecord/coin_boxes/0" target="_blank">here</a>.</small>
			</div>
		</div>
		<div class="row">
			<div class="span12">
...
?>
	...
	<script type="text/javascript">
		function supportedmethods() {
			...
			var bitcoin_currencies = new Array(<?php echo '"'.implode('", "', $bitcoin_currency_list).'"'; ?>);
			var interkassa_currencies = new Array(<?php echo '"'.implode('", "', $interkassa_currency_list).'"'; ?>);
			...
			if (jQuery.inArray(currency, interkassa_currencies) >= 0) supported = supported + '<span class="label label-success">InterKassa</span> ';
			...
			if (jQuery.inArray(currency, bitcoin_currencies) >= 0) supported = supported + '<span class="label label-success">Bitcoin</span> ';
...
<?php		
		...
		<tr>
			...
			<td style="text-align: right;">'.number_format($row['gross'], ($row['currency'] == 'BTC' ? 8 : 2), ".", "").' '.$row['currency'].'</td>
...
<div class="navbar navbar-fixed-bottom navbar-inverse">
	<div class="navbar-inner">
		<div class="container">
			...
			<p class="navbar-text pull-right">Copyright &copy; 2011-<?php echo date('Y'); ?> <a href="http://codecanyon.net/user/halfdata/portfolio?ref=halfdata" target="_blank">Halfdata, Inc.</a></p>
		</div>
	</div>
</div>
<!-- Footer - end -->
